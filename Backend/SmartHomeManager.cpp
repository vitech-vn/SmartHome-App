...
void SmartHomeManager::slot_handleTurnOnDevice(const std::string &id)
{
    ...
    std::vector<AlertInfo> overloadAlerts = alertSystem.checkOverload(summary.totalCurrentPower);
    for (const AlertInfo &alert : overloadAlerts)
    {
...
void SmartHomeManager::slot_handleTurnOffDevice(const std::string &id)
{
    ...
    device->turnOff();
    alertSystem.clearDeviceAlerts(id);
